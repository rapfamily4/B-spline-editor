...
signal knot_generation_finished(knots)
signal knot_vector_sorted(knots)
...
var spline_degree: int = 2 setget set_spline_degree # k
var spline_resolution: int = 96 setget set_spline_resolution
var knots: Array # It must be normalized in the interval [0, 1].
var knots_generation_mode: int = KnotsGenerationMode.Unclamped
var show_curve_connection_markers: bool = false setget set_show_curve_connection_markers
var evaluation_min: float = -INF setget set_evaluation_min # a
var evaluation_max: float = INF setget set_evaluation_max # b


func _ready():
	show_curve_connection_markers = show_curve_connection_markers
	var window_resolution: Vector2 = get_viewport_rect().size
	for i in range(CONTROL_POINTS_COUNT):
		var new_x: float = lerp(-window_resolution.x * 0.2, window_resolution.x * 0.2, float(i) / float(CONTROL_POINTS_COUNT - 1))
		var new_y: float = rand_range(-window_resolution.y * 0.075, window_resolution.y * 0.075)
...
func add_control_point(point_position: Vector2 = Vector2.ZERO) -> void:
	var new_point: Position2D = control_point_scene.instance()
	new_point.position = point_position
	new_point.connect("control_point_moved", self, "update_curve")
	control_points_tree.add_child(new_point)

func generate_knots() -> void:
	knots.clear()
	for i in range(CONTROL_POINTS_COUNT + spline_degree + 1):
		if knots_generation_mode != KnotsGenerationMode.Unclamped:
			if i in range(spline_degree + 1):
				...
				var sum: float = 0
				for j in range(i - spline_degree, i):
	...
	for marker in knot_markers_tree.get_children():
		marker.free()
	for i in range(knots.size() - 2 * spline_degree - 2):
		knot_markers_tree.add_child(knot_marker_scene.instance())
	
	update_curve()
	emit_signal("knot_generation_finished", knots)
...
func set_knot(index: int, value: float) -> void:
	...
	evaluation_min = knots[spline_degree]
	evaluation_max = knots[CONTROL_POINTS_COUNT]
	emit_signal("knot_vector_sorted", knots)
	update_curve()

func update_curve() -> void:
	line_renderer.clear_points()
	for i in range(spline_resolution):
		var evaluation_point: float = lerp(evaluation_min, evaluation_max, float(i) / float(spline_resolution - 1))
		line_renderer.add_point(evaluate_curve(evaluation_point))
	
	for i in range(spline_degree + 1, knots.size() - spline_degree - 1):
		var marker: Position2D = knot_markers_tree.get_child(i - spline_degree - 1)
...
func evaluate_curve(evaluation_point: float) -> Vector2:
	...
	for j in range(spline_degree, CONTROL_POINTS_COUNT):
...
# Setget
func set_spline_degree(value):
	assert(value >= 0)
	spline_degree = value

func set_spline_resolution(value):
	assert(value >= 0)
	spline_resolution = value

func set_show_curve_connection_markers(value):
	knot_markers_tree.visible = value
	show_curve_connection_markers = value

func set_evaluation_min(value):
	assert(value >= knots[spline_degree])
	evaluation_min = value

func set_evaluation_max(value):
